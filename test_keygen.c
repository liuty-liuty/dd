    ...
    /* Now, we test out various parameter sets (both legal and not) */
    {
        ...
        for (i=0; i<3; i++) {
            ...
            do {
                ...
                bool expect_success;
                enum hss_error_code expected_code = hss_error_bad_param_set;
                    /* If we expect an error, this is the error we expect */
                switch (lm) {
                ...
                case LMS_SHAKE_N24_H15:
                    expect_success = do_sha3;
                    expected_code = hss_error_sha3_not_enabled;
                    break;
                ...
                if (!got_success && hss_extra_info_test_error_code(&info) !=
                                                 expected_code) {
                    printf( "Bad parm set got incorrect error (%d, %d)\n",
...
next_iter:      lm ^= 0xe0000000L;  /* Cycle through the e000000 parm sets */
        ...
        for (i=0; i<3; i++) {
            ...
            do {
                ...
                if (!got_success &&
                             hss_extra_info_test_error_code(&info) != expected_code) {
                    printf( "Bad parm set got incorrect error (%d vs %d)\n",
                            hss_extra_info_test_error_code(&info), expected_code );
                    return false;
                }
    ...
    static struct {
        param_set_t p;
        enum { normal=0, sha3_only=1, slow=2 } flag;
    } parms_to_test[] = {
        { LMS_SHA256_N32_H5 },
        { LMS_SHA256_N24_H5 },
        { LMS_SHA256_N32_H10 },
        ...
        { LMS_SHA256_N32_H15 },
        { LMS_SHA256_N24_H15 },
        { LMS_SHAKE_N32_H5, sha3_only },
        { LMS_SHAKE_N24_H5, sha3_only },
        { LMS_SHAKE_N32_H10, sha3_only },
        { LMS_SHAKE_N24_H10, sha3_only },
        { LMS_SHAKE_N32_H15, slow | sha3_only },
        { LMS_SHAKE_N24_H15, slow | sha3_only },
        { LMS_SHA256_N32_H20, slow },
        { LMS_SHA256_N24_H20, slow },
        { LMS_SHAKE_N32_H20, slow | sha3_only },
        { LMS_SHAKE_N24_H20, slow | sha3_only },
    };
    for (i=0; i<sizeof parms_to_test/sizeof *parms_to_test; i++) {
        /* If we don't support SHA3, don't test those parameter sets */
        if (!do_sha3 && (parms_to_test[i].flag & sha3_only)) {
            continue;
        }

        /* In fast mode, don't try the slow parm sets */
        if (fast_flag && (parms_to_test[i].flag & slow)) {
            continue;
        }

        param_set_t lm_type[2];
        lm_type[0] = parms_to_test[i].p; lm_type[1] = LMS_SHA256_N24_H5;
